    ...
    <el-row class="mx-[-10px]">
      ...
      <el-col :span="24" class="px-10px">
        <el-form-item prop="deptId">
          <el-select v-model="registerData.registerForm.deptId" placeholder="申请部门"
            size="large" class="!w-full" :loading="optionsLoading">
            <el-option v-for="d in registerOptions.depts" :key="d.id" :label="d.name" :value="d.id" />
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :span="24" class="px-10px">
        <el-form-item prop="postId">
          <el-select v-model="registerData.registerForm.postId" placeholder="申请职位"
            size="large" class="!w-full" :loading="optionsLoading" clearable>
            <el-option v-for="p in registerOptions.posts" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :span="24" class="px-10px">
...
const registerOptions = ref<{ depts: { id: number; name: string }[]; posts: { id: number; name: string }[] }>({
  depts: [],
  posts: []
})
...
const loadRegisterOptions = async () => {
  ...
  try {
    registerOptions.value = await LoginApi.getRegisterOptions()
  } finally {
...
const handleRegister = async (params: any) => {
  ...
  try {
    ...
    // 注册 = 提交待审批申请；管理员审批通过后账号才生效
    await LoginApi.register(registerData.registerForm)
    message.success('注册申请已提交，等待管理员审批通过后方可登录')
    ...
    registerData.registerForm.deptId = undefined
    registerData.registerForm.postId = undefined
    handleBackLogin()
  } finally {
