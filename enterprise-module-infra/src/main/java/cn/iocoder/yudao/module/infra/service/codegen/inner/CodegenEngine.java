package com.enterprise.module.infra.service.codegen.inner;
...
import com.enterprise.framework.apilog.core.enums.OperateTypeEnum;
...
import com.enterprise.framework.common.util.date.LocalDateTimeUtils;
...
import com.enterprise.framework.excel.core.annotations.DictFormat;
import com.enterprise.framework.excel.core.convert.DictConvert;
...
import com.enterprise.module.infra.enums.codegen.CodegenVOTypeEnum;
...
import static com.enterprise.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.enterprise.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 enterprise-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 enterprise-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.enterprise.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/enterprise-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/enterprise-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "enterprise-module-${table.moduleName}/" + // 顶级模块
                "enterprise-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "enterprise-module-${table.moduleName}/" + // 顶级模块
                "enterprise-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "enterprise-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "enterprise-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "enterprise-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "enterprise-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
